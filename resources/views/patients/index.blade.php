                ...
                <div class="px-8 py-8 bg-gradient-to-r from-purple-500 to-purple-600 border-b border-purple-200/50">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-4xl font-bold text-white flex items-center">
                            <div class="w-14 h-14 mr-5 bg-purple-500 rounded-2xl flex items-center justify-center backdrop-blur-sm">
                            ...
                            <div>
                                <div class="text-4xl text-gray-800 font-bold">Patient Records</div>
                                <div class="text-gray-700 text-lg font-medium">Manage your healthcare data</div>
                            </div>
                        </h3>
                            ...
                            <form method="GET" action="{{ route('patients.index') }}" class="w-80">
                                ...
                                <div class="relative group">
                                    <div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                                        <svg class="w-5 h-5 text-gray-400 group-focus-within:text-purple-500 transition-colors" aria-hidden="true"
                    ...
                    <div class="overflow-x-auto">
                        <table id="patientsTable" class="min-w-full divide-y divide-gray-200 text-center">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Nº</th>
                                    <th scope="col" class="px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Name</th>
                                    <th scope="col" class="px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">DOB</th>
                                    <th scope="col" class="px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Phone</th>
                                    <th scope="col" class="px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Address</th>
                                    <th scope="col" class="px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Gender</th>
                                    <th scope="col" class="px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                                ...
                                @if ($patients->count() > 0)
                                    @foreach ($patients as $patient)
                                        <tr class="hover:bg-gray-50 transition-all duration-200">
                                           <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-900">{{ $patients->count() - $loop->index }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                                {{ $patient->first_name }} {{ $patient->last_name }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $patient->date_of_birth }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $patient->phone }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 max-w-xs truncate">{{ $patient->address }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $patient->gender === 'Male' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-pink-50 text-pink-700 border border-pink-200' }}">
                                                    {{ $patient->gender }}
                                                </span>
                                            </td>
                                                    ...
                                                    <div id="deleteModal-{{ $patient->_id }}"
                                                        ...
                                                        <div class="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-md border border-gray-100">
                                                            <div class="text-center">
                                                                ...
                                                                <p class="text-gray-600 mb-8">Are you sure you want to delete this patient? <br>This action cannot be undone.</p>
                                                                <div class="flex justify-center space-x-4">
                                                                    ...
                                                                    <form action="{{ route('patients.destroy', $patient->_id) }}" method="POST">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="submit"
                                                                            class="px-8 py-3 bg-red-500 text-white rounded-2xl hover:bg-red-600 transition-colors duration-200 font-medium">Delete</button>
                                                                    </form>
                                                                </div>
                                                            </div>
                                                        </div>
                                    ...
                                    <tr id="no-results">
                                        <td colspan="7" class="text-center py-16">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <p class="text-xl font-semibold text-gray-600 mb-2">No patients found</p>
                                                <p class="text-gray-500">Get started by adding your first patient</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endif
                    ...
                    noResults.innerHTML = `
                        <td colspan="7" class="text-center py-16">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-xl font-semibold text-gray-600 mb-2">No patients found matching</p>
        ...
        document.getElementById('default-search').addEventListener('input', function() {
            if (this.value === '') {
                ...
                rows.forEach(row => {
                    ...
                    cells.forEach(cell => {
                        if (cell.innerHTML.includes('<span class="bg-emerald-100 text-emerald-800')) {
